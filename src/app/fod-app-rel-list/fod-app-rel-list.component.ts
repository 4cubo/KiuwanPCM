...
import { FoDApplication, FoDService } from '../_services/fortify.service';
...
export class FoDAppRelListComponent implements OnInit, AfterViewInit {
	...
	getAllAppRel( appId : string, req : any = null, initInfo: boolean = false) {
		...
		if  ( req == null ){
			...
			req = {
				method: 'GET',
				url:  "/applications/" + appId + "/releases?orderBy=releaseCreatedDate&orderByDirection=DESC&offset=" + this.appOffset + "&limit=" + this.appLimit
			}
		}
